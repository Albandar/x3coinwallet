...
void SendFrame::amountValueChange() {
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      ...
      float donationpercent = amount * 0.5 / 100; // donation is 0.5%
...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("x3coin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "x3coin:");
    }
    if(!_request.startsWith("x3coin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with x3coin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "X3CzbgGGNW4ewWTa4iFphKehf3JvLY9UW2iyAMmasaC5LqcfMtaavhrhWDhzvdvGT6C7HL697wxFSL2xTCiKgHpbVF6S3GJoDx7";
