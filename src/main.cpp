...
// Copyright (c) 2016-2017 The X3Coin developers
...
int main(int argc, char* argv[]) {
  ...
  QSettings x3coinKey("HKEY_CLASSES_ROOT\\x3coin", QSettings::NativeFormat);
  x3coinKey.setValue(".", "X3Coin Wallet");
  x3coinKey.setValue("URL Protocol", "");
  QSettings x3coinOpenKey("HKEY_CLASSES_ROOT\\x3coin\\shell\\open\\command", QSettings::NativeFormat);
  x3coinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = X3Coin URL Handler\\nGenericName = X3Coin\\nComment = Handle URL Sheme x3coin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/x3coin;\\nIcon = X3Coin-Wallet' | tee /usr/share/applications/x3coin-handler.desktop";
